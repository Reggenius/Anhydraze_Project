...
@section('content')
    ...
    @endif
    <form id="timetable_form"  action="{{ route('user.notes.edit', $id) }}" method="POST">
        @csrf
        <div class="img_table_div">
            <div class="tabledescribe">Description:</div>
            <textarea id="describe" name="note_description" placeholder="Optional" maxlength="100" autofocus>{{ $resource->note_description }}</textarea>
    ...
    <div class="timetableDel">
        <a href="{{ asset($resource->note_url) }}" download><input type="submit" value="Download" id="submit2" /></a>
        <a href="{{ route('user.notes.delete', $id) }}" ><input type="submit" value="Delete" id="submit2" /></a>
    </div><hr>
@endsection
<!-- end of content -->